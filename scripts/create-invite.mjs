...
async function main() {
  ...
  if (!dbUrl) {
    console.log("[invite] failed: DATABASE_URL is not set.");
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`[invite] expires: ${expiresAt.toISOString()}`);
  } finally {
...
main().catch((err) => {
  console.log(`[invite] failed: ${err instanceof Error ? err.message : String(err)}`);
  process.exitCode = 1;
});
